...
function Landing({ onStart, onLogin }) {
  return (
    <div className="cam-screen cam-landing">
      ...
      <div className="cam-floating-hearts" aria-hidden="true">
        <Heart className="cam-heart-float cam-heart-float--1" size={18} fill="currentColor" />
        <Heart className="cam-heart-float cam-heart-float--2" size={14} fill="currentColor" />
        <Heart className="cam-heart-float cam-heart-float--3" size={22} fill="currentColor" />
        <Heart className="cam-heart-float cam-heart-float--4" size={12} fill="currentColor" />
        <Heart className="cam-heart-float cam-heart-float--5" size={16} fill="currentColor" />
      </div>
      <div className="cam-landing-content">
        <div className="cam-logo-mark cam-logo-mark--landing cam-fade-up" style={{ animationDelay: "0ms" }}>
          <img src={CAM_LOGO_URI} alt="CAM" className="cam-logo-img" />
        </div>
        <p className="cam-eyebrow cam-fade-up" style={{ animationDelay: "80ms" }}>
          Centre d'Amitié et de Mariage
        </p>
        <h1 className="cam-hero-title cam-fade-up" style={{ animationDelay: "160ms" }}>
          Là où deux <em>horizons</em>
          <br />
          se rencontrent.
        </h1>
        <p className="cam-hero-sub cam-fade-up" style={{ animationDelay: "240ms" }}>
        ...
        <div className="cam-horizon-strip cam-fade-up" aria-hidden="true" style={{ animationDelay: "320ms" }}>
        ...
        <button
          className="cam-btn-primary cam-btn-full cam-btn-glow cam-fade-up"
        ...
        <button
          className="cam-btn-ghost cam-btn-ghost--light cam-fade-up"
      ...
      {loading ? (
        ...
        <>
          <div className="cam-card-stack">
            {showHeartBurst && (
              <div className="cam-heart-burst" aria-hidden="true">
...
function MatchOverlay({ me, match, onKeepBrowsing, onSendMessage }) {
  const confettiColors = ["#E8607A", "#F0A868", "#7BC6E8", "#8B5FBF", "#FFD9C7"];
  return (
    <div className="cam-match-overlay">
      <div className="cam-confetti" aria-hidden="true">
        {Array.from({ length: 24 }).map((_, i) => (
          <span
            key={i}
            className="cam-confetti-piece"
            style={{
              left: `${(i * 97) % 100}%`,
  ...
  return (
    <style>{`
      ...
      * { box-sizing: border-box; }
      ...
      /* ---------- Animations d'accroche ---------- */
      @keyframes cam-fade-up-kf {
        from { opacity: 0; transform: translateY(14px); }
        to { opacity: 1; transform: translateY(0); }
      }
      .cam-fade-up { opacity: 0; animation: cam-fade-up-kf 0.6s cubic-bezier(0.16,1,0.3,1) forwards; }

      @keyframes cam-glow-pulse-kf {
        0%, 100% { box-shadow: 0 8px 20px rgba(232,96,122,0.35); }
        50% { box-shadow: 0 8px 30px rgba(232,96,122,0.65), 0 0 0 6px rgba(232,96,122,0.12); }
      }
      .cam-btn-glow { animation: cam-fade-up-kf 0.6s cubic-bezier(0.16,1,0.3,1) forwards, cam-glow-pulse-kf 2.2s ease-in-out 1.2s infinite; }

      .cam-floating-hearts { position: absolute; inset: 0; overflow: hidden; pointer-events: none; z-index: 0; }
      .cam-heart-float { position: absolute; color: rgba(255,255,255,0.35); animation: cam-float-kf linear infinite; }
      .cam-heart-float--1 { left: 12%; top: 85%; animation-duration: 9s; animation-delay: 0s; }
      .cam-heart-float--2 { left: 78%; top: 90%; animation-duration: 11s; animation-delay: 1.2s; }
      .cam-heart-float--3 { left: 45%; top: 95%; animation-duration: 13s; animation-delay: 2.4s; color: rgba(240,168,104,0.4); }
      .cam-heart-float--4 { left: 25%; top: 88%; animation-duration: 8s; animation-delay: 3.6s; }
      .cam-heart-float--5 { left: 62%; top: 92%; animation-duration: 12s; animation-delay: 0.6s; color: rgba(240,168,104,0.35); }
      @keyframes cam-float-kf {
        0% { transform: translateY(0) rotate(0deg); opacity: 0; }
        10% { opacity: 1; }
        90% { opacity: 1; }
        100% { transform: translateY(-620px) rotate(20deg); opacity: 0; }
      }

      @keyframes cam-heart-burst-kf {
      ...
      .cam-heart-burst {
        position: absolute; top: 42%; left: 50%; z-index: 20;
        color: var(--cam-rose);
        filter: drop-shadow(0 6px 14px rgba(232,96,122,0.5));
        animation: cam-heart-burst-kf 0.65s cubic-bezier(0.16,1,0.3,1) forwards;
        pointer-events: none;
      }

      .cam-confetti { position: absolute; inset: 0; overflow: hidden; pointer-events: none; z-index: 1; }
      .cam-confetti-piece {
        position: absolute; top: -10%; width: 8px; height: 14px; border-radius: 2px;
        animation-name: cam-confetti-fall-kf; animation-timing-function: ease-in; animation-fill-mode: forwards;
      }
      @keyframes cam-confetti-fall-kf {
