...
async function loadSession() {
  try {
    const raw = localStorage.getItem("cam:session");
...
async function saveSession(session) {
  try {
    localStorage.setItem("cam:session", JSON.stringify(session));
  } catch {
    /* best effort */
  }
}
async function clearSession() {
  try {
    localStorage.removeItem("cam:session");
  } catch {
    /* ignore */
  }
}
...
function Discover({ profiles, loading, likesCount, onOpenLikes, onDecision, onOpenProfile }) {
...
export default function CamApp() {
  ...
  const refreshDiscover = async (activeSession, myId) => {
    setLoadingQueue(true);
    try {
      const rows = await sbFrom(
        `profiles?id=neq.${myId}&select=*&order=created_at.desc&limit=30`,
        activeSession
      );
